...
<div class="col-md-5"><label class="form-label">Docente *</label><select name="idDocente" class="form-select" required><option value="">Seleccionar...</option><%for(Docente d:docs){if(docentesOcupados.contains(d.getId()))continue;%><option value="<%=d.getId()%>"><%=d.getNombreCompleto()%></option><%}%></select>
<% if(docentesOcupados.size()>=docs.size()){ %><div class="form-text text-danger">Todos los docentes ya tienen aula asignada para este examen.</div><% } %></div>
<div class="col-md-5"><label class="form-label">Aula *</label><select name="idAula" class="form-select" required><option value="">Seleccionar...</option><%for(Aula a:aulas){if(aulasOcupadas.contains(a.getId()))continue;%><option value="<%=a.getId()%>"><%=a.getCodigo()%> (<%=a.getCapacidad()%> cupos)</option><%}%></select>
<% if(aulasOcupadas.size()>=aulas.size()){ %><div class="form-text text-danger">Todas las aulas ya tienen docente asignado para este examen.</div><% } else if(!aulasOcupadas.isEmpty()){ %><div class="form-text text-muted">Las aulas ya asignadas no aparecen en la lista.</div><% } %></div>
